        ...
        while not rospy.is_shutdown():
            ...
            if interacting_with_remote_controller is None:  # check if we have a remote controller state change and send flips
                ...
                if interacting_with_remote_controller == rocon_app_manager_msgs.Constants.NO_REMOTE_CONTROLLER:
                    response = self.kill_turtle(concert_service_msgs.KillTurtleRequest(self.name), timeout=rospy.Duration(4.0))
                else:
                    response = self.spawn_turtle(concert_service_msgs.SpawnTurtleRequest(self.name), timeout=rospy.Duration(4.0))
