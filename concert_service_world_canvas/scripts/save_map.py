#!/usr/bin/env python
'''
    Concert version of save_map.
    Uses rocon_python_comms.find_topic to get world_canvas handle 
'''
...
if __name__ == '__main__':
    ...
    try:
        known_save_annotations = rocon_python_comms.find_service('world_canvas_msgs/SaveAnnotationsData', timeout=rospy.rostime.Duration(5.0), unique=True)
    except rocon_python_comms.NotFoundException as e:
        self.logerr("could not locate the scheduler's known resources topic [%s]" % str(e))
        sys.exit(1)

    save_srv = rospy.ServiceProxy(known_save_annotations, world_canvas_msgs.srv.SaveAnnotationsData)
